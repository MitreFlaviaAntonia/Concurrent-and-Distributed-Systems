...
public class GrandSorcererHelper implements Runnable {
    ...
    private static void WitchesCreator(int witchIndex, Witch[] witch) {
    ...
    private static void UndeadCreator(int undeadIndex, Undead[] undead) {
        ...
        if(undeadIndex <= grandSorcerersCircle.getUndeadsNumber()) { //the maximum of undeads
            ...
            Thread[] t = new Thread[grandSorcerersCircle.getUndeadsNumber()];
            t[undeadIndex] = new Thread(undead[undeadIndex]);
            t[undeadIndex].start();
        }
    }

    static void retireDemons(int destroyedDemonsNumber, int UndeadIndex, int CovenIndex) {
