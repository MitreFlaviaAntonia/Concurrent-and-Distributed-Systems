...
public class GrandSorcererHelper implements Runnable {
    ...
    GrandSorcerersCircle getGrandSorcerersCircle() {
        return grandSorcerersCircle;
    }
    ...
    private static void WitchesCreator(int witchIndex, Witch[] witch) { // creates witches

        if(witchIndex <= 200) { //200 is the maximum of witches

            witch[witchIndex] = new Witch();
            Thread[] t = new Thread[200]; //a new thread is created, for every witch
            t[witchIndex] = new Thread(witch[witchIndex]);
            t[witchIndex].start();
        }
    }

    private static void UndeadCreator(int undeadIndex, Undead[] undead) { // creates undeads
        ...
        if(undeadIndex <= grandSorcerersCircle.getUndeadsNumber()) { //the maximum of undeads
            ...
            Thread[] t = new Thread[grandSorcerersCircle.getUndeadsNumber()]; //a new thread is created, for every undead
            t[undeadIndex] = new Thread(undead[undeadIndex]);
            t[undeadIndex].start();
        }
    }

    static void retireDemons(int destroyedDemonsNumber, int UndeadIndex, int CovenIndex) { // retires demons
