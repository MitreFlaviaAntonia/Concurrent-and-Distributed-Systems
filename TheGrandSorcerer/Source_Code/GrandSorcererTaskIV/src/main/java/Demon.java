...
public class Demon implements Runnable {
    ...
    private int CreationRestrict = 0; //restrict ingredient creation for a demon (nr of turns restricted)
    private int WallHitsCounter = 0; //wall hits counter for a demon
    private int SocialSkillsLevel = 0; //social skills level for a demon
    ...
    private void diagonalAreaCheck(Demon demon, int Line, int Column) {
        if(Line == Column) { //diagonal area
            demon.isAsleep = true; //if the demon is in the diagonal area he should sleep
            demon.covens[CovenIndex].asleepCounter++;
        }
    }
    ...
    private void RandomSpawn(Demon demon, int CovenIndex) { // a demon random spawns in a coven
        ...
        if(demon.covens[CovenIndex].CovenMatrix[Line][Column] == 0) { //checks is the position is free (if there is not already other demon)
            ...
            diagonalAreaCheck(demon, Line, Column); //check if the demon is in the diagonal area
        }
        else
    ...
    private int RandomMove(Demon demon, int CovenIndex, int Line, int Column) {
        ...
        if (covens[CovenIndex].CovenMatrix[Line][Column] == 0) { //if it is free
            ...
            diagonalAreaCheck(demon, Line, Column);
    ...
    private void dropIngredients(int CovenIndex, int IngredientIndex) {
        ...
        covens[CovenIndex].DecreaseIngredientCounter(IngredientIndex, covens[CovenIndex].getIngredientsCount().get(IngredientIndex));//the demons drop all the existing inredients
    }
            ...
            while (!isRetired) {
                ...
                while(!covens[CovenIndex].StopDemons()) {
                    ...
                    try {
                        ...
                        grandSorcerersCircle.getSleeping()[CovenIndex].await(); //the barrier increases the number of sleeping demons with 1
                    }
