...
public class  GrandSorcerersCircle  {
    ...
    public static GrandSorcererHelper GrandSorcererHelper;

    public static void setGrandSorcererHelper(GrandSorcererHelper grandSorcererHelper) {
        GrandSorcererHelper = grandSorcererHelper;
    }

    public GrandSorcererHelper getGrandSorcererHelper () {
        return GrandSorcererHelper;
    }

    public static CyclicBarrier[] sleeping;

    public static void setSleeping(CyclicBarrier[] Sleeping) {
        sleeping = Sleeping;
    }

    public CyclicBarrier[] getSleeping() {
        return sleeping;
    }
    ...
    public static void main(String[] args) {
        ...
        CovensNumber = randomNumberGenerator.generateRandomNumber(1,3); //here the coven number is generated
        ...
        sleeping = new CyclicBarrier[CovensNumber];
        for(int i=0; i<CovensNumber; i++) { //creates the covens and the matrices of the covens
            covens[i] = new Coven();
            covens[i].CovenSize = randomNumberGenerator.generateRandomNumber(20,50); //here the size of coven i is generated
            sleeping[i] = new CyclicBarrier(covens[i].CovenSize);
        ...
        Thread t = new Thread(grandSorcererHelper); //a new thread is created, for every philosopher
        t.start();

        DiagonalArea diagonalArea = new DiagonalArea(); //a new diagonal area object is created
        Thread p = new Thread(diagonalArea); // a thread p is created using it
