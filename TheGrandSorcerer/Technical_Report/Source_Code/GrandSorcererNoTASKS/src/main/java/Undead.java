...
public class Undead implements Runnable {
    ...
    private void RandomVisit(int UndeadIndex, int CovenIndex) {
        ...
        if(!getFighting()) {
            if(covens[CovenIndex].getDemonsNumber()>10) { //if there are more than 10 demons at the moment in a coven
            ...
            if(!isDefeated) {
                covens[CovenIndex].loseIngredients(0.1, CovenIndex); //lose 10% of the ingredients from the coven
            }
            else
            {
